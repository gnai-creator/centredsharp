...
public partial class HeightMapGenerator
{
    ...
    {
        bool openLandGroups = ImGui.BeginChild("LandGroups", new System.Numerics.Vector2(0, 140), ImGuiChildFlags.Borders);
        if (openLandGroups)
        {
            foreach (var kv in groups.ToArray())
            {
                ...
                if (ImGui.Selectable($"{kv.Key} (Ch: {kv.Value.ChannelIndex}, {kv.Value.Chance:0.#}% )", isSel))
                    selected = kv.Key;
        ...
        if (ImGui.Button("Add##l"))
        {
            ...
            {
                groups.Add(newName, new Group(newName));
                selected = newName;
        ...
        if (!string.IsNullOrEmpty(selected) && groups.TryGetValue(selected, out var grp))
        {
            ...
            int channel = grp.ChannelIndex;

            ImGui.DragInt("Min Height", ref minH, 1, -128, 127);
            ImGui.DragInt("Max Height", ref maxH, 1, -128, 127);
            ImGui.DragInt("Channel Index", ref channel, 0, 0, NUM_CHANNELS - 1);

            grp.MinHeight = (sbyte)minH;
            grp.MaxHeight = (sbyte)maxH;
            grp.ChannelIndex = channel;

            bool openTiles = ImGui.BeginChild($"{selected}_tiles", new System.Numerics.Vector2(0, 100), ImGuiChildFlags.Borders);
            if (openTiles)
            {
